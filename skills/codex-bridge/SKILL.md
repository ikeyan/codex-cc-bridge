...
6. token ファイルは**削除せずセッション中保持する**。「起動後に削除して秘密をメモリだけにする」
   案は堅牢化にならない: 削除すると以後 Claude が token を持ち回ることになり、
   (a) token が会話コンテキスト経由でセッショントランスクリプト (~/.claude/projects/) に永続化される、
   (b) 毎ターンの再具現化でコマンドライン (ps) に露出する、(c) コンテキスト圧縮で token を失うと
   誰も認証できない server がポートを占有するロックアウトになる。ファイル保持なら token 文字列は
   一度もデータとして持ち回られない (Claude が知るのはパスのみ)。なお同一ユーザーの能動的な
   攻撃者は ~/.codex の資格情報ごと読めるため、どの方式でも防げない (スコープ外)。
