...
var ConnectionHandler = class {
	...
	applyEditQueue = Promise.resolve();
	...
	processBuffer() {
		for (;;) {
			if (this.state.contentLength === -1) {
				const joined$1 = this.joinChunks();
				const headerEnd = joined$1.indexOf("\r\n\r\n");
				if (headerEnd === -1) return;
				const header = joined$1.subarray(0, headerEnd).toString("utf8");
				const contentLengthMatch = header.match(/Content-Length: (\d+)/);
				this.setChunks(joined$1.subarray(headerEnd + 4));
			...
			if (this.state.bufferedBytes < this.state.contentLength) return;
			const joined = this.joinChunks();
			const messageBody = joined.subarray(0, this.state.contentLength).toString("utf8");
	...
	joinChunks() {
		if (this.state.chunks.length > 1) this.state.chunks = [Buffer.concat(this.state.chunks)];
	...
	handleNotificationOrRequest(message) {
		...
		} else if (isLSPRequest(message)) if (message.method === "workspace/applyEdit") this.handleApplyEdit(message);
		else if (message.method === "client/registerCapability") {
			const registrations = message.params?.registrations ?? [];
			const unsupported = registrations.map((r) => r.method).filter((method) => !HONOURED_REGISTRATIONS.has(method));
			if (unsupported.length === 0) this.sendResponse(message.id, null);
			else this.sendError(message.id, -32602, `Cannot honour registration of: ${unsupported.join(", ")}`);
		} else if (ACKNOWLEDGED_SERVER_REQUESTS.has(message.method)) this.sendResponse(message.id, null);
	...
	handleApplyEdit(message) {
		const handler = this.handlers.applyEdit;
		if (!handler) {
			this.sendError(message.id, -32601, "Method not found: workspace/applyEdit");
			return;
		}
		const edit = message.params?.edit;
		if (!edit) {
			this.sendError(message.id, -32602, "workspace/applyEdit: params.edit is required");
			return;
		}
		this.applyEditQueue = this.applyEditQueue.then(() => handler(edit)).then((result) => this.sendResponse(message.id, result), (error$2) => this.sendError(message.id, -32603, error$2 instanceof Error ? error$2.message : String(error$2))).catch((error$2) => debug("workspace/applyEdit response failed:", error$2));
	}
...
async function applyWorkspaceEdit(edit, fileSystemApi) {
	if ("documentChanges" in edit && edit.documentChanges !== void 0) return {
		applied: false,
	...
	try {
		for (const [uri, edits] of Object.entries(edit.changes ?? {})) {
			...
			await fileSystemApi.writeFile(filePath, applyTextEdits(currentContent, edits));
		}
		return { applied: true };
	} catch (err) {
		return {
...
*/
async function applyEditFromServer(edit, fileSystemApi, documentManager, sendNotification) {
	...
	for (const uri of Object.keys(edit.changes ?? {})) {
		...
		if (documentManager.isDocumentOpen(uri)) documentManager.updateDocument(uri, content, sendNotification);
	}
	return result;
}
...
function createLSPClient(config) {
	...
	const connection = new ConnectionHandler(state, { applyEdit: (edit) => applyEditFromServer(edit, state.fileSystemApi, documentManager, (m, p) => connection.sendNotification(m, p)) });
