...
var ConnectionHandler = class {
	...
	processBuffer() {
		while (this.state.buffer.length > 0) {
			...
			if (bytes.length < this.state.contentLength) return;
			const messageBody = bytes.subarray(0, this.state.contentLength).toString("utf8");
			this.state.buffer = bytes.subarray(this.state.contentLength).toString("utf8");
