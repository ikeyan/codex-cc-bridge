...
import { debugLog } from "./debugLog-LfbHS9a2.js";
import { dependencyMajor } from "./packageVersion-DCtYpxhL.js";
import { existsSync } from "fs";
import { join } from "path";

//#region src/utils/environmentDetector.ts
function detectEnvironment(root) {
	debugLog("environment", `Detecting environment in ${root}`);
	if (existsSync(join(root, "deno.json")) || existsSync(join(root, "deno.jsonc"))) return {
		preset: "deno",
		confidence: "high",
	...
	if (existsSync(join(root, "tsconfig.json"))) {
		...
		if (existsSync(packageJsonPath)) try {
			...
			if (packageJson.devDependencies?.["tsgo"] || packageJson.dependencies?.["tsgo"]) return {
				preset: "tsgo",
				confidence: "high",
				reason: "Found tsconfig.json with tsgo in dependencies",
				suggestedFiles: ["**/*.ts", "**/*.tsx"]
			};
			const typescriptMajor = dependencyMajor(root, packageJson, "typescript");
			if (typescriptMajor !== void 0 && typescriptMajor >= 7) return {
				preset: "tsgo",
				confidence: "high",
				reason: `Found tsconfig.json with typescript ${typescriptMajor} (native tsc --lsp) in dependencies`,
				suggestedFiles: ["**/*.ts", "**/*.tsx"]
			};
			if (packageJson.devDependencies?.["typescript"] || packageJson.dependencies?.["typescript"]) return {
				preset: "typescript",
				confidence: "high",
	...
	if (existsSync(join(root, "pyproject.toml")) || existsSync(join(root, "setup.py")) || existsSync(join(root, "requirements.txt"))) return {
		preset: "pyright",
		confidence: "high",
		reason: "Found Python project files (pyproject.toml/setup.py/requirements.txt)",
		suggestedFiles: ["**/*.py", "**/*.pyi"]
	};
	if (existsSync(join(root, "*.fsproj")) || existsSync(join(root, "*.sln"))) return {
	...
	if (existsSync(join(root, "moon.mod.json"))) return {
		preset: "moonbit",
		confidence: "high",
		reason: "Found moon.mod.json",
		suggestedFiles: ["**/*.mbt"]
	};
	if (existsSync(join(root, "gleam.toml"))) return {
		preset: "gleam",
		confidence: "high",
...
function formatEnvironmentGuide(detected) {
	const lines = [
	...
	lines.push(``, `To use this preset, create .lsmcp/config.json with:`, ``, `{`, `  "preset": "${detected.preset}"`);
	if (detected.suggestedFiles && detected.suggestedFiles.length > 0) lines.push(`  "files": ${JSON.stringify(detected.suggestedFiles, null, 2).split("\n").join("\n  ")}`);
	lines.push(`}`);
	return lines.join("\n");
}

//#endregion
export { detectEnvironment, formatEnvironmentGuide };