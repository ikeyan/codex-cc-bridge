...
var ConnectionHandler = class {
	...
	handleApplyEdit(message) {
		...
		this.applyEditQueue = this.applyEditQueue.then(() => handler(edit)).then((result) => this.sendResponse(message.id, result), (error$2) => this.sendError(message.id, -32603, error$2 instanceof Error ? error$2.message : String(error$2)));
	}
...
async function applyEditFromServer(edit, fileSystemApi, documentManager, sendNotification) {
	...
	for (const uri of Object.keys(edit.changes ?? {})) if (documentManager.isDocumentOpen(uri)) {
