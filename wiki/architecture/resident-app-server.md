...
実際のポート (`listening on: ws://127.0.0.1:NNNNN`) を `ready` が読んでセッションディレクトリの
`port` に書く。driver はそのディレクトリ (`--session DIR` / `CODEX_BRIDGE_SESSION`) から token と
port の両方を読み、endpoint は常に `ws://127.0.0.1:<port>` に組む。**driver に既定ポートは無く、
session が無ければ起動を拒否する。** host や URL を受けるフラグも無いので、token が loopback 以外に
送られる argv 経路は存在しない。
...
既定ポートを残したまま port 0 を使うと、`--session` を忘れた呼び出しが黙って
