...
`review/start` の target も同じで、モードだけを `--review` フラグにし、ブランチ名や sha は
`--target-file` の 1 行ファイルから、指示文は stdin から読む。ブランチ名は `$(...)` や `'` を
含みうる (git が受け付けることを実測) ので、値を引数で受けるフラグは意図的に無い。
